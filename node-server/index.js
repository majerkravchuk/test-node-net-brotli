...
const patchSocket = (socket) => {
    ...
    const before = new PassThrough({
        transform: (chunk, encoding, callback) => {
            console.log(`chunk size before: ${chunk.length}`);
            callback(null, chunk);
        }
    });
    const after = new PassThrough({
        transform: (chunk, encoding, callback) => {
            console.log(`chunk size after: ${chunk.length}`);
            callback(null, chunk);
        }
    });

    before.pipe(compression).pipe(after).pipe(socket);
    socket.writeCompressed = (message) => {
        before.write(message, 'binary', (err) => {
            if (err) {
...
export const onSocketReadeble = (socket) => {
    ...
    while ((message = socket.read(4)) !== null) {
        ...
        if (message.readInt32BE() === 0xF) {
            sendStringMessage(socket, `Server response number ${++counter} is that ${RESPONSE}`);
        }
    }
};
